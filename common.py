...
import json
import pprint
url='http://10.50.6.150/'#'http://202.121.178.214/' #'http://10.50.6.150/'
def fun_exe(resource_dict,fun_name,api_key):
	data_dict = urllib.quote(json.dumps(resource_dict))

	request = urllib2.Request(url+'api/3/action/' + fun_name)
...
def datastore_create(resource_id,api_key):
	...
	resource_dict = {
    	...
    	"force": True,
    	"fields": [{"id":"date","type":"text"},{"id":"transmitter","type":"text"},{"id":"head","type":"text"},{"id":"data","type":"text"}],
    	#[{"id": "author", "type": "text"}, {"id": "submitted_on", "type": "timestamp"}, {"id": "PM2_5", "type": "float"}, {"id": "CO", "type": "float"}, {"id": "PM10", "type": "float"}, {"id": "SO2", "type": "float"}, {"id": "O3", "type": "float"}, {"id": "NO2", "type": "float"}],
    	# 'resource': resource,
...
def url_update(resource_id,api_key):
	...
	resource_dict = {
    	'id': resource_id,
    	'url': url+'datastore/dump/' + resource_id,
    	'revision_id': '1.1'
	}
...
resource_id = '216fae5a-f3ce-4905-b119-93d9a975cec6'
api_key = '6bc98f2a-c9cb-4c31-86d6-b6c924413180'